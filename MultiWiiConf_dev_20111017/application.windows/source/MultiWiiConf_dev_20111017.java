...
public class MultiWiiConf_dev_20111017 extends PApplet {
...
public void READ() {
  ...
  for(int i=0;i<5;i++) {confP[i].setValue(byteP[i]/10.0f);confI[i].setValue(byteI[i]/1000.0f);confD[i].setValue(byteD[i]);}
  ...
  for(int i=0;i<8;i++) for(int a=0;a<6;a++)
    if ((PApplet.parseByte(activation[i])&(1<<a))>0) checkbox[i].activate(a); else checkbox[i].deactivate(a);
...
public void WRITE() {
  ...
  int p = 0;
   ...
   for(int i =0;i<33;i++)    g_serial.write(PApplet.parseChar(s[i]));
}
...
class cGraph {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#ECE9D8", "MultiWiiConf_dev_20111017" });
  }
}
